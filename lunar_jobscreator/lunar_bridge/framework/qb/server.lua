...
function player:addItem(name, count, metadata)
    ...
    if ox_inventory then
        ...
        return true
    elseif codemInventory then
        local ok, result = pcall(function() return exports['codem-inventory']:AddItem(self.source, name, count, nil, metadata) end)
        return ok and result or false
    else
        local ok, result = pcall(function() return self.QBPlayer.Functions.AddItem(name, count, nil, metadata) end)
        return ok and result or false
    end
end
